...
/// A type that defines the options used by an `ExecutableEntryPoint` so it knows how to identify a config file or a dry run of a config file.
public protocol ConfigOptionsSettings {
    ...
    /// The long option that should be used to pass in a config file for this executable.
    ///
    /// If this is passed it will always take presidence over the `autoConfig` option.
    ///
    /// - Note: This defaults to `"config"`
    static var config: String { get }

    /// The help message that should be attached to the `config` option.
...
extension ConfigOptionsSettings {
    @inlinable
