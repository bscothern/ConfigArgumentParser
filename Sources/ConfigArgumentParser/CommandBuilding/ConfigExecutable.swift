...
/// ```
///
/// You are only required to provide an interpreter and options settings when desired.
///
...
///
/// The default `ConfigOptionsSettings` uses `--config [config_path]` to find the config file when supplied and `--config-dry-run` flag to print out what would have been run if just `--config [config_path]` were supplied.
///
/// - Note: Because of how `ArgumentParser` generates its help commands the the config flags are entirely hidden and cannot match options in your normal command otherwise `ConfigExecutable` will not work and just your command will be run with those arguments.
...
extension ConfigExecutable: ConfigFileInterpreterCustomizable {
    @inlinable
    public static func interpretConfig(with interpreter: ConfigFileInterpreter.Type) -> (ConfigOptionsCustomizable & ExecutableEntryPoint).Type {
        interpreter.bind(to: RootCommand.self)
    }
}

extension ConfigExecutable: ConfigOptionsCustomizable {
    @inlinable
    public static func customizeOptions(with customizeOptions: ConfigOptionsSettings.Type) -> ExecutableEntryPoint.Type {
        Self.interpretConfig(with: Interpreters.default)
            .customizeOptions(with: customizeOptions)
    }
}
