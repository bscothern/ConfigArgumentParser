...
//        let files = try configFiles(for: name)
    }
    
    
    func configFiles(for example: String) throws -> (good: String, bad: String) {
        (
            good: try String(contentsOfFile: Self.packageRoot + example + "/config_good"),
            bad: try String(contentsOfFile: Self.packageRoot + example + "/config_bad")
        )
    }
